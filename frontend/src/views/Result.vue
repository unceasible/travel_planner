  ...
  <div class="result-container">
    ...
    <div v-if="tripPlan" class="content-wrapper">
        ...
        <a-card title="📅 每日行程" :bordered="false" class="days-card">
          <a-collapse v-model:activeKey="activeDays" accordion>
            ...
            >
              ...
              <!-- 交通安排 -->
              <a-divider orientation="left">🚕 交通安排</a-divider>
              <a-list
                :data-source="day.transport_segments || []"
                size="small"
                bordered
              >
                  ...
                  <a-list-item>
                    <div style="width: 100%;">
                      ...
                      <div style="color: #666; font-size: 13px;">
                        {{ item.mode }} · {{ formatDistance(item.distance) }} · {{ formatDuration(item.duration) }}
                      </div>
              ...
              <div class="day-budget">
                <div class="day-budget-title">当日预算小计</div>
                <div class="day-budget-grid">
                  <span>门票 ¥{{ day.day_budget?.attractions || 0 }}</span>
                  <span>餐饮 ¥{{ day.day_budget?.meals || 0 }}</span>
                  <span>住宿 ¥{{ day.day_budget?.hotel || 0 }}</span>
                  <span>交通 ¥{{ day.day_budget?.transportation || 0 }}</span>
                </div>
                <div class="day-budget-total">合计: ¥{{ day.day_budget?.subtotal || 0 }}</div>
              </div>
            </a-collapse-panel>
          </a-collapse>
        </a-card>
      ...
      <div class="chat-panel">
        <a-card title="💬 智能修改" :bordered="false" class="chat-card">
          <div class="chat-meta">
            <div>任务ID: {{ taskId || '未绑定' }}</div>
            <div>更新模式: {{ updateMode || '-' }}</div>
          </div>
          ...
          <a-button
            ...
            :loading="chatLoading"
            :disabled="!chatInput.trim() || !taskId"
...
onMounted(async () => {
  ...
  try {
    if (taskId.value) {
      const response = await getTripTask(taskId.value)
      if (response.success && response.data) {
        tripPlan.value = response.data
        updateMode.value = response.update_mode || 'restore'
        if (response.user_id) {
...
const sendChatMessage = async () => {
  ...
  if (!content || !taskId.value) return
  ...
  try {
    ...
    if (response.success && response.data) {
      tripPlan.value = response.data
      updateMode.value = response.update_mode || ''
      if (response.assistant_message) {
        chatHistory.value.push({ role: 'assistant', content: response.assistant_message })
        sessionStorage.setItem('tripAssistantMessage', response.assistant_message)
      }
      sessionStorage.setItem('tripPlan', JSON.stringify(response.data))
      sessionStorage.setItem('tripUpdateMode', updateMode.value)
      if (response.user_id) {
        userId.value = response.user_id
        sessionStorage.setItem('tripUserId', userId.value)
      }
